...
#define	TMR_ARRAY_DIM			10		/* dimensione massima array dei timers */
...
void wait(u32 ms)
{
	tmr_wait = ms;      	// iniz.variabile di ritardo
	while(tmr_wait != 0);	// attesa esaurimento tempo
} /* WAIT */
...
void TMR_Start(tmr *tim, u32 set)
{
	...
	{
		Error_Handler();	// locazioni esaurite!!!
	}
