...
void	SCP_Init(void)
{
	...
	rx[PORT1].usart	= USART2;
	rx[PORT1].dma_rx = DMA1_Channel6;
	rx[PORT1].dma_tx = DMA1_Channel7;
  ...
  /* USART pins configurations */
	...
	io_param.GPIO_Pin = GPIO_Pin_3;
	io_param.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOA, &io_param);
	io_param.GPIO_Mode = GPIO_Mode_AF_PP;
	io_param.GPIO_Pin = GPIO_Pin_2;
	GPIO_Init(GPIOA, &io_param);
	...
	dma_param.DMA_PeripheralBaseAddr = (u32)(&USART2->DATAR);
	...
	DMA_Init(DMA1_Channel7, &dma_param);
  DMA_Cmd(DMA1_Channel7, ENABLE); /* USART1 Tx */
	...
	DMA_Init(DMA1_Channel6, &dma_param);
  DMA_Cmd(DMA1_Channel6, ENABLE); /* USART1 Rx */
