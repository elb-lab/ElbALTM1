...
#ifdef __SPI_MODE__
#define MS_SPI_MOSI_PIN			PA7
#define MS_SPI_MISO_PIN			PA6
#define MS_SPI_SCLK_PIN			PA5
#define MS_SPI_CSEL_PIN			PA4
...
void	MS5611_InitConfig(void)
{
	...
	io_param.GPIO_Pin = GPIO_Pin_5 | GPIO_Pin_7;
	GPIO_Init(GPIOA, &io_param);
	/* NSS Pin */
	__SPI_CS_DISABLE__();
	io_param.GPIO_Mode = GPIO_Mode_Out_PP;
	io_param.GPIO_Pin = GPIO_Pin_4;
	GPIO_Init(GPIOA, &io_param);
	/* MISO Pin */
	...
	/* SPI Configuration */
	spi_param.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_64;
...
void	MS5611_SendCmd(u8 cmd)
{
  ...
  while(SPI_I2S_GetFlagStatus(MS_SPI_PORT, SPI_I2S_FLAG_TXE) == RESET);
	...
	SPI_I2S_ReceiveData(MS_SPI_PORT);
	wait(10);
  __SPI_CS_DISABLE__();
}
